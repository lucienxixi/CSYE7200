...
import edu.neu.coe.csye7200.asstfc.Movie.MoviesProtocol.{moviesJsonFormat}
...
object Movie extends App {
  ...
  object MoviesProtocol extends DefaultJsonProtocol {
    // 20 points
    // TO BE IMPLEMENTED
    implicit val nameJsonFormat: RootJsonFormat[Name] = jsonFormat4(Name.apply)
    implicit val ratingJsonFormat: RootJsonFormat[Rating] = jsonFormat2(Rating.apply)
    implicit val formatJsonFormat: RootJsonFormat[Format] = jsonFormat4(Format.apply)
    implicit val productionJsonFormat: RootJsonFormat[Production] = jsonFormat4(Production.apply)
    implicit val reviewsJsonFormat: RootJsonFormat[Reviews] = jsonFormat7(Reviews.apply)
    implicit val principalJsonFormat: RootJsonFormat[Principal] = jsonFormat2(Principal.apply)
    implicit val moviesJsonFormat: RootJsonFormat[Movie] = jsonFormat11(Movie.apply)
  }
