...
//    }// TO BE IMPLEMENTED
  ...
  /**
    ...
    * @return a value of R, wrapped in Try
    */
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = t1y flatMap (t1 => t2y flatMap (t2 => t3y map (t3 => f(t1, t2, t3))))// TO BE IMPLEMENTED
...
//                                         (f: (T1, T2, T3, T4, T5, T6, T7) => R): Try[R] = t1y flatMap (t1 => t2y flatMap (t2 => t3y flatMap( t3 => t4y flatMap (t4 => t5y flatMap (t5 => t6y flatMap (t6 => t7y map (t7 => f(t1, t2, t3, t4, t5, t6, t7))))))))// TO BE IMPLEMENTED
  ...
  /**
    ...
    */
  // You know this one
//  def lift[T, R](f: T => R): Try[T] => Try[R] = _ match {
//    case Success(t) => Success(f(t))
//    case Failure(e) => Failure(e)
//  } // TO BE IMPLEMENTED
//  def lift[T, R](f: T => R): Try[T] => Try[R] = t1 => t1.map(f) // TO BE IMPLEMENTED
...
//  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = (t1,t2) => map2(t1, t2)(f) // TO BE IMPLEMENTED
  ...
  def invert2[T1, T2, R](f: T1 => T2 => R): T2 => T1 => R = (t2: T2) => (t1: T1) => f(t1)(t2) // TO BE IMPLEMENTED
